...
REM ----------------------------------------------------------------------------
SET CFN_STACK_NAME=TastylogRDS
SET CFN_TEMPLATE=51_rds_create_TastylogRDS.yml
...
FOR /F "usebackq delims=" %%A in (
  ...
  SET SSM_MYSQL_PASSWORD=%%A
)
REM エラーハンドリング
REM FOR /F 内のコマンドでエラーが発生した場合 ERRORLEVEL, %ERRORLEVEL% は 0 なので判断できない
REM ここでは do 後の環境変数に値が設定されたかを判定条件とする
IF NOT DEFINED SSM_MYSQL_PASSWORD (
  ECHO Failed: aws ssm get-parameter
  GOTO :ERREND
) ELSE (
  ECHO Success: aws ssm get-parameter
)
